...
% load example data
load('/scratch/ywang330/SBCI_AVG/CogTE1001/dwi_pipeline/sbci_connectome/RUN001/fc_avg_ico4.mat')
load('/scratch/ywang330/SBCI_AVG/CogTE1001/dwi_pipeline/sbci_connectome/smoothed_sc_avg_0.005_ico4.mat')

% load required SBCI data for mapping and analysis
[sbci_parc, sbci_mapping, adjacency] = load_sbci_data('/scratch/ywang330/SBCI_AVG', 'ico4');
...
sbci_parc.atlas
my_ROI_list = sbci_parc(12).names; %ROI names

%for Desikan, ROIs 1 and 36 are 'LH_missing' and 'RH_missing'. So let's remove them 
my_ROI_list([1,36],:) = []; %updated ROI names

dct_fc = parcellate_fc(fc, sbci_parc(12), sbci_mapping, 'roi_mask', [1,36]);
dct_sc = parcellate_sc(sc, sbci_parc(12), sbci_mapping, 'roi_mask', [1,36]);

%% Plot Continuous Connectivity
plot_sbci_mat(fc, sbci_parc(12), 'roi_mask', [1,36], 'figid', 1, 'clim', [-0.1, 0.35]);
title('Continuous FC (Desikan)')

plot_sbci_mat(log((10^7*sc) + 1), sbci_parc(12), 'roi_mask', [1,36], 'figid', 2, 'clim', [0, 3.5]);
